...
for _ in range(5):
    frame_stack.append(np.zeros(num_obs, dtype=np.float32))
    mujoco.mj_step(m, d)
...
while d.time < warmup:
    ...
    d.ctrl[:] = tau
    mujoco.mj_step(m, d)
print("after warmup: t=%.2f h=%.3f vx=%.3f" % (d.time, d.qpos[2], d.qvel[0]))

while d.time < 12.0 and d.qpos[2] > 0.35:
    tau = pd_control(target_dof_pos, d.qpos[7:], kps, np.zeros_like(kds), d.qvel[6:], kds)
    d.ctrl[:] = tau
    mujoco.mj_step(m, d)
    if int((d.time-warmup)*500) % control_decimation == 0:
        qj = d.qpos[7:]
        dqj = d.qvel[6:]
        quat = d.qpos[3:7]
        ...
        obs[9:38] = (qj - default_angles) * dof_pos_scale
        obs[38:67] = dqj[policy_to_xml] * dof_vel_scale
    ...
    if int((d.time-warmup)*500) % 500 == 0:
        log.append((d.time, d.qpos[2], d.qvel[0]))

print("end: h:", round(d.qpos[2], 3), "vx:", round(d.qvel[0], 3))
for row in log:
    print("t=%.2f h=%.3f vx=%.3f" % row)