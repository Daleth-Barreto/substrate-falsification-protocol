...
falsification battery \cite{carmena2003,embodied_neurocomp}. Gate~G
...
\begin{itemize}[nosep]
\item Code: \path{sandbox/} git repo; Gates B/B2/C/D/D\,bis/E/F/G runners
(\path{gate_b/run.py}, \path{04_p2/gate_b2/run3.py}, \path{gate_d/run.py},
\path{gate_d/run_mlp.py}, \path{gate_d/run_20seeds.py},
\path{gate_d/run_archB.py}, \path{gate_e/run.py}) print JSON summaries
into their \path{results/};
...
Gate~G (architecture~B): \path{gate_d/run_archB.py} patches the hub
geometry (\path{N}, \path{p_re}) and writes
\path{gate_d/results_archB/}; the architecture~A artifacts are untouched.
